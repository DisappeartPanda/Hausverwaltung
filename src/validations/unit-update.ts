...
const ALLOWED_STATUSES: UnitUpdateInput["status"][] = [
  "frei",
  "vermietet",
  "reserviert",
];
...
export function validateUnitUpdateInput(
  input: unknown,
): ValidationResult<UnitUpdateInput> {
  ...
  if (!unitId) {
    errors.unitId = "Bitte gib eine gültige Wohneinheit an.";
  }
  ...
  if (!ALLOWED_STATUSES.includes(status as UnitUpdateInput["status"])) {
  ...
  return {
    ...
    data: {
      ...
      title,
      objectId,
      status: status as UnitUpdateInput["status"],
    },
  };
}